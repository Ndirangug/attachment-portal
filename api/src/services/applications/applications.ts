...
export const Application = {
  student: (_obj, { root }: ResolverArgs<ReturnType<typeof application>>) =>
    db.application.findUnique({ where: { id: root.id } }).student(),
  opportunity: (_obj, { root }: ResolverArgs<ReturnType<typeof application>>) =>
    db.application.findUnique({ where: { id: root.id } }).opportunity(),
  Placement: (_obj, { root }: ResolverArgs<ReturnType<typeof application>>) =>
    db.application.findUnique({ where: { id: root.id } }).Placement(),
}
